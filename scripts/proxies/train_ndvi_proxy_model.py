...
RUN_ID = f"train_ndvi_proxy_model_{RUN_TS}"

DATASET_PATH = DATASETS_DIR / "cmip6_ndvi_proxy_training_dataset_20260322T125749Z.parquet"

MODEL_PATH = MODELS_DIR / f"ndvi_proxy_model_{RUN_TS}.joblib"
METRICS_PATH = MANIFESTS_DIR / f"{RUN_ID}_metrics.json"

FEATURES = ["t2m", "d2m", "sm", "pet"]
TARGET = "ndvi_obs"
TIME_COL = "yyyymm"
...
# Time-based split
train_df = df[df["_yyyymm_int"] <= 202012].copy()
valid_df = df[(df["_yyyymm_int"] >= 202101) & (df["_yyyymm_int"] <= 202602)].copy()
...
print(f"Valid MAE     : {metrics['valid_mae']:.6f}")
print(f"Valid RMSE    : {metrics['valid_rmse']:.6f}")
